...
🔑 *Chave PIX:* \`${pixKey}\` (${cantina.pixReceiverName || cantina.name})
...
*Valor:* R$ ${totalDebt.toFixed(2)}`;
  ...
  return (
    <div className="fixed inset-0 bg-slate-950/85 backdrop-blur-sm flex items-center justify-center p-3 z-50 overflow-y-auto">
      <div className="bg-slate-900 border border-slate-700 rounded-2xl max-w-lg w-full shadow-2xl overflow-hidden my-auto animate-in fade-in zoom-in-95 duration-200">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-3.5 bg-slate-800/90 border-b border-slate-700">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="font-bold text-white text-sm">Cobrança Automática no WhatsApp</h3>
        ...
        <div className="p-4 sm:p-5 space-y-4 max-h-[80vh] overflow-y-auto">
          ...
          {/* Profile selector buttons matching video */}
          <div>
            <label className="block text-xs font-semibold text-slate-300 mb-2">
              Escolha o estilo da mensagem:
            </label>
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <span className="text-xs font-semibold text-slate-300">
                Prévia do texto que o cliente vai receber:
              </span>
              <button
            ...
            <div className="bg-slate-950 border border-slate-800 rounded-xl p-3.5 text-xs text-slate-200 whitespace-pre-wrap font-sans leading-relaxed max-h-48 overflow-y-auto border-l-4 border-l-emerald-500 shadow-inner">
              {messageText}
            </div>
          </div>
        </div>
